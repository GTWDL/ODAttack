...
# def check_patchsize(ref_path, adv_path):
#     import cv2

#     img_original = cv2.imread(ref_path)
#     img_patched = cv2.imread(adv_path)
#     gray_original = cv2.cvtColor(img_original, cv2.COLOR_BGR2GRAY)
#     gray_patched = cv2.cvtColor(img_patched, cv2.COLOR_BGR2GRAY)

#     diff = cv2.absdiff(gray_original, gray_patched)
#     thresh = cv2.threshold(diff, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
#     patch_area = np.count_nonzero(diff)
#     # patch_area = cv2.countNonZero(thresh)
#     print(patch_area)
#     return patch_area <=49152
